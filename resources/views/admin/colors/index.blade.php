...
@section('title') {{ trans('admin.colors') }} @endsection
    ...
    <section>
        <div class="card">
            <div class="card-header">
                <div class="tbl-title">{{ trans('admin.colors') }}</div>
            </div>
            <hr>
    ...
    $(document).ready(function(){
        $('#colors-table').DataTable({
            ...
            buttons: [
                ...
                { text: '<i class="feather icon-plus"></i> {{ trans("admin.create_color") }}',
                  className: '@if (auth()->user()->can("create_colors")) btn dtbtn btn-sm btn-primary @else btn dtbtn btn-sm btn-primary disabled @endif',
                  attr: {
                          title: '{{ trans("admin.create_color") }}',
                          href: '{{ route("admin.colors.create") }}' 
                        },
                    action: function (e, dt, node, config)
                    {
                        // href location
                        window.location.href = '{{ route("admin.colors.create") }}';
                    }
                },
            ],
            language: {
