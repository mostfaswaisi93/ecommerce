...
@section('title') {{ trans('admin.malls') }} @endsection
    ...
    <section>
        <div class="card">
            <div class="card-header">
                <div class="tbl-title">{{ trans('admin.malls') }}</div>
            </div>
            <hr>
            <div class="card-content">
                <div class="card-body">
                    <div class="table-responsive">
                        <table id="malls-table" class="table table-striped table-bordered dt-responsive nowrap">
                            <thead>
                                <tr>
                                    <th>#</th>
...
<script type="text/javascript">
    $(document).ready(function(){
        $('#malls-table').DataTable({
            processing: true,
            serverSide: true,
            responsive: true,
            order: [[ 2, "desc" ]],
            ajax: {
                url: "{{ route('admin.malls.index') }}",
            },
            columns: [{
            ...
            buttons: [
                ...
                { text: '<i class="feather icon-plus"></i> {{ trans("admin.create_mall") }}',
                  className: '@if (auth()->user()->can("create_malls")) btn dtbtn btn-sm btn-primary @else btn dtbtn btn-sm btn-primary disabled @endif',
                  attr: {
                          title: '{{ trans("admin.create_mall") }}',
                          href: '{{ route("admin.malls.create") }}' 
                        },
                    action: function (e, dt, node, config)
                    {
                        // href location
                        window.location.href = '{{ route("admin.malls.create") }}';
                    }
                },
            ],
            language: {
    ...
    $(document).on('click', '.delete', function(){
        ...
        swal({
            ...
            if(result.value){
                $.ajax({
                    url:"malls/destroy/" + mall_id,
                    success: function(data){
                        $('#malls-table').DataTable().ajax.reload();
